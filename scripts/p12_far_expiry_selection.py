...
def main() -> None:
    ...
    OUT_CONCLUSION.write_text('\n'.join(['# P12 Final Research Conclusion','',decision,'','The primary adaptive rule selects the far expiry using only 09:15 entry information: maximum normalized entry credit among F+1 to F+4 eligible candidates. Fixed-horizon and adaptive OOS results are reported in P12_FAR_EXPIRY_SUMMARY.csv and P12_FAR_EXPIRY_COST_SENSITIVITY.csv.','', 'No further far-expiry search or score-weight optimization is performed in P12.']),encoding='utf-8')
